...
- [ ] **Stale `:latest` tags are now frozen in both backend repos.** `Realm-ID/api`,
      ...
      **Capability shipped 2026-08-13 (v2.2.0): `cleanup-gar-images` gained `delete_tags`.**
      Still OPEN because nothing has been applied. Next: dispatch each caller
      (`Realm-ID/project`, `Traide-Co/project`) with `dry_run: true` and `delete_tags: latest`
      — that is the ONLY read path for a keep-set digest, so it is the first confirmation that
      these tags are really still there. Then re-dispatch with `dry_run: false`. Note the
      digest is reclaimed on the NEXT scheduled sweep, not the same run, by design.
      Callers pass `delete_tags` only on `workflow_dispatch` — the reusable ignores it on a
      schedule, but the caller should not offer it on one either.
...
- [ ] **`SERVICE_ACCOUNT` is referenced but never set in `cleanup-gar-images.yml`.** Two
      ...
      permission. The `delete_tags` step added 2026-08-13 sets it in its own `env:`; these two
