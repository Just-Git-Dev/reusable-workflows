...
| build | `context`, `dockerfile`, `platforms`, `build_args`, `provenance` (`mode=min`), `sbom` (`false`), `cache_scope`, `dry_run`; secret `build_secrets` (newline `id=value` → build-push `secrets:`, e.g. `gh_pat=<PAT>` for private modules via `RUN --mount=type=secret,id=gh_pat`). `provenance`/`sbom` reproduce buildx's own default and are stated here so they survive an action bump |
...
jobs:
  deploy:
    uses: Just-Git-Dev/reusable-workflows/.github/workflows/deploy-cluster-keyed.yml@v2.10.0
    with:
      registry_type: ecr
