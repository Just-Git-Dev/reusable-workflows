...
def test_repo_is_internally_consistent():
    ...
    # assertion a tautology. Bump it when a reusable is ADDED (23 as of
    # deploy-cloudflare-worker, 2026-08-24) — if it fails without one being added,
    # a workflow has silently stopped being stamped, which is the point.
    check("repo: stamped 23 reusables", len(report.stamps), 23)
