...
def test_repo_is_internally_consistent():
    ...
    # assertion a tautology. Bump it when a reusable is ADDED (24 as of
    # bootstrap-cf-service, 2026-08-24) — if it fails without one being added,
    # a workflow has silently stopped being stamped, which is the point.
    check("repo: stamped 24 reusables", len(report.stamps), 24)
